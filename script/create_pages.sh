#!/bin/bash
# ============================================================================================
# Bash Shell Script for creation of carousel elements.
# Naming scheme for carousel elements: page<INDEX>.png where INDEX starts from 0.
# The script creates a style entry in cards.css for each carousel element found in the 
# resources/images folder.
# ============================================================================================

declare -r CONTENT_PATH=../client/9Pad/resources/images
declare -r OUTPUT_FILE=../client/9Pad/resources/css/cards.css

echo "Creating $OUTPUT_FILE..."

echo "/****************************" > $OUTPUT_FILE
echo "* Carousel elements." >> $OUTPUT_FILE
echo "* This file is generated by create_pages.sh in the scripts folder." >> $OUTPUT_FILE
echo "*/" >> $OUTPUT_FILE

ls $CONTENT_PATH/page*.png | sed "s/.*\///" | while read F
do
  LEN=${#F}
  INDEX=${F:4:LEN-8}
   
  echo ".myContentCard$INDEX {" >> $OUTPUT_FILE
  echo "  background: url('../images/page$INDEX.png') no-repeat center;" >> $OUTPUT_FILE
  echo "}" >> $OUTPUT_FILE
done

echo "done."
